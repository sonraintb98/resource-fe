...
const getErrorMessage =
  ...
  (field: LoginFormFields): string | undefined => {
    if ((form.formState.errors[field] as unknown as FieldError[])?.length) {
      return (form.formState.errors[field] as unknown as FieldError[]).map((error) => error.message).join('\n');
    }
...
export function useFormSchema() {
  const formSchema: yup.SchemaOf<LoginRequest> = yup.object({
    ...
    password: yup.string().required('Password is required.').min(5, 'Password should minimum 5 characters.'),
  });
